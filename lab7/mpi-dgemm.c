...
#define MATRIX_SIZE 8  // 可以修改为 2, 4, 8，以进行不同的测试  

void print_matrix(double matrix[MATRIX_SIZE][MATRIX_SIZE], const char *name) {  
    printf("Matrix %s:\n", name);  
...
int main(int argc, char **argv) {  
    ...
    double C[MATRIX_SIZE][MATRIX_SIZE] = {0}; // Initialize C to 0  
    ...
    if (size != 1) {  
        fprintf(stderr, "This program should run with a single process.\n");  
